...
class SmartReceiptAnalyzer {
  ...
  async analyzeReceiptText(text: string, confidence: number): Promise<SmartAnalysisResult> {
    ...
    const [amounts, dates, merchant, category, fuelInfo] = await Promise.all([
      ...
      this.extractFuelInfo(processedLines)
    ]);

    const [fuelLiters, fuelType, kilometers] = fuelInfo;
  ...
  /**
   * Calcolo categoria finale
   */
  private calculateFinalCategory(categories: { category: string; confidence: number }[]): { category: string; confidence: number } {
    // Implementazione di esempio, da adattare in base alla logica desiderata
    // Per ora, restituisce la categoria con la confidenza più alta
    if (categories.length === 0) {
      return { category: 'Altro', confidence: 0.1 }; // Categoria di default
    }

    // Calcolo categoria finale
    const finalCategory = categories.reduce((best, current) =>
      current.confidence > best.confidence ? current : best
    );
  ...
  /**
   * Estrae informazioni su carburante e kilometraggio
   */
  private async extractFuelInfo(lines: string[]): Promise<[number | undefined, string | undefined, number | undefined]> {
    let liters: number | undefined;
    let type: string | undefined;
    let km: number | undefined;

    // Pattern per litri
    // Es: 20,50 l, 20.50 lt, Litri 20,50
    const literPatterns = [
      /(?:litri|lt|lit|l)\.?\s*[:]?\s*(\d{1,3}(?:[.,]\d{1,2})?)/i,
      /(\d{1,3}(?:[.,]\d{1,2})?)\s*(?:litri|lt|lit|l)(?![a-z])/i
    ];
    ...
    for (const line of lines) {
      ...
      if (!liters) {
        for (const pattern of literPatterns) {
          ...
          if (match) {
            const val = this.parseAmountToNumber(match[1]);
            if (val > 0 && val < 1000) { // check range ragionevole per auto
              liters = val;
              break;
            }
          }
      ...
      // Cerca tipo carburante
      if (!type) {
        for (const fuel of fuelTypes) {
          if (fuel.keywords.some(k => lowerLine.includes(k))) {
            type = fuel.type;
            break;
          }
        }
